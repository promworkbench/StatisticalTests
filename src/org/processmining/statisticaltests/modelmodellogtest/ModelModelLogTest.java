...
public class ModelModelLogTest implements
		StatisticalTest<Triple<StochasticLabelledPetriNet, StochasticLabelledPetriNet, XLog>, ModelModelLogTestParameters> {
	...
	public double test(Triple<StochasticLabelledPetriNet, StochasticLabelledPetriNet, XLog> input,
			final ModelModelLogTestParameters parameters, ProMCanceller canceller, Progress progress)
			throws InterruptedException {
		final StochasticLabelledPetriNet netA = input.getA();
		final StochasticLabelledPetriNet netB = input.getB();
		final XLog log = input.getC();
		...
		new ConcurrentSamples<SplittableRandom>(parameters.getThreads(), parameters.getNumberOfSamples(), canceller,
			...
			protected boolean performSample(SplittableRandom random, int sampleNumber, ProMCanceller canceller) {
				...
				XLog sampleA;
				try {
					sampleA = StochasticLabelledPetriNetSample.sample(netA, parameters.getSampleSize(), random,
							canceller);
				...
				XLog sampleB;
				try {
					sampleB = StochasticLabelledPetriNetSample.sample(netB, parameters.getSampleSize(), random,
							canceller);
				} catch (ModelHasZeroWeightsException e1) {
					e1.printStackTrace();
					return false;
	...
	public double testUnitEMSC(Triple<StochasticLabelledPetriNet, StochasticLabelledPetriNet, XLog> input,
			final ModelModelLogTestParameters parameters, ProMCanceller canceller, Progress progress)
			throws InterruptedException {
		final StochasticLabelledPetriNet netA = input.getA();
		final StochasticLabelledPetriNet netB = input.getB();
