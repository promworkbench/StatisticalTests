 ...
 *
 * An implementation of the alias method implemented using Vose's algorithm. The
 * alias method allows for efficient sampling of random values from a discrete
 * probability distribution (i.e. rolling a loaded die) in O(1) time each after
 ...
 * 
 * Updated by Sander Leemans for primitive double array rather than List<Double>
 */
...
public final class AliasMethod {
	...
	public AliasMethod(double[] probabilities, Random random) {
		 ...
		 * changes to it.
		 */
		probabilities = ArrayUtils.clone(probabilities);
